...
2. Accepted enabling value: exactly `"true"` in [`isAuthEnabled()`](/home/ccod/repos/bsr-contract-chatbot/src/lib/supabase/config.ts:7)
3. Default when absent: disabled
4. What enabling changes:
   - `/` stops rendering the anonymous chat directly and requires a Supabase user session before loading the app shell and persisted thread state in [`Home`](/home/ccod/repos/bsr-contract-chatbot/src/app/page.tsx:9)
...
8. Dependencies on other flags:
   ...
   - operational dependency: when enabled, Supabase config must also be valid through `NEXT_PUBLIC_SUPABASE_URL` and either `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` or `NEXT_PUBLIC_SUPABASE_ANON_KEY` in [`getSupabaseUrl()`](/home/ccod/repos/bsr-contract-chatbot/src/lib/supabase/config.ts:11) and [`getSupabasePublishableKey()`](/home/ccod/repos/bsr-contract-chatbot/src/lib/supabase/config.ts:19)
...
10. Exact source files/symbols that read it:
   ...
   - [`proxy.ts` `proxy()`](/home/ccod/repos/bsr-contract-chatbot/proxy.ts:9)
   - [`src/lib/chat-session.ts` `resolveChatSession()`](/home/ccod/repos/bsr-contract-chatbot/src/lib/chat-session.ts:50)

## `ENABLE_ANSWER_VALIDATION_LOGGING`

1. Exact variable name: `ENABLE_ANSWER_VALIDATION_LOGGING`
2. Accepted enabling value: exactly `"true"` in [`isAnswerValidationLoggingEnabled()`](/home/ccod/repos/bsr-contract-chatbot/src/lib/evidence/answer-validation-logging.ts:48)
3. Default when absent: disabled
4. What enabling changes:
   - after the candidate answer already exists, `/api/chat` runs the offline validator against the final answer plus canonical evidence in [`POST` on stream finish](/home/ccod/repos/bsr-contract-chatbot/src/app/api/chat/route.ts:1791)
   - when validation reaches a `validated` result, the server emits a log line starting with `[chat] answer-validation ` in [`logAnswerValidationResult()`](/home/ccod/repos/bsr-contract-chatbot/src/lib/evidence/answer-validation-logging.ts:132)
5. What enabling does not change:
...
10. Exact source files/symbols that read it:
   - [`src/lib/evidence/answer-validation-logging.ts` `isAnswerValidationLoggingEnabled()`](/home/ccod/repos/bsr-contract-chatbot/src/lib/evidence/answer-validation-logging.ts:48)
   - [`src/app/api/chat/route.ts` stream `onFinish`](/home/ccod/repos/bsr-contract-chatbot/src/app/api/chat/route.ts:1791)
...
2. Accepted enabling value: exactly `"true"` in [`isLocalTraceCaptureEnabled()`](/home/ccod/repos/bsr-contract-chatbot/src/lib/evidence/local-answer-trace.ts:94)
3. Default when absent: disabled
4. What enabling changes:
...
10. Exact source files/symbols that read it:
   - [`src/lib/evidence/local-answer-trace.ts` `isLocalTraceCaptureEnabled()`](/home/ccod/repos/bsr-contract-chatbot/src/lib/evidence/local-answer-trace.ts:94)
...
2. Accepted enabling value: not a boolean; it must parse to an allowed loopback `http` URL in [`getLocalTraceCaptureEndpoint()`](/home/ccod/repos/bsr-contract-chatbot/src/lib/evidence/local-answer-trace.ts:103)
3. Default when absent: unset, therefore invalid and skipped
4. What enabling changes:
   - when the two boolean gates are also enabled, `/api/chat` POSTs the raw local trace payload to this endpoint in [`sendLocalAnswerTrace()`](/home/ccod/repos/bsr-contract-chatbot/src/lib/evidence/local-answer-trace.ts:286)
   - the receiving route validates and writes trace files beneath `.local/traces/` in [`writeRawLocalAnswerTrace()`](/home/ccod/repos/bsr-contract-chatbot/src/lib/evidence/local-trace-file-writer.ts:319)
5. What enabling does not change:
...
9. Security or operational warnings:
   ...
   - accepted forms are restricted to `http`, a non-empty port, no username, no password, no query string, no hash, path exactly `/api/local-traces`, and hostname limited to `127.0.0.1`, `localhost`, or `::1` in [`getLocalTraceCaptureEndpoint()`](/home/ccod/repos/bsr-contract-chatbot/src/lib/evidence/local-answer-trace.ts:120)
   - do not enable this on an externally reachable deployment
10. Exact source files/symbols that read it:
   - [`src/lib/evidence/local-answer-trace.ts` `getLocalTraceCaptureEndpoint()`](/home/ccod/repos/bsr-contract-chatbot/src/lib/evidence/local-answer-trace.ts:103)

Operational notes for local trace capture:

- raw traces are written beneath `.local/traces/` in [`src/lib/evidence/local-trace-file-writer.ts`](/home/ccod/repos/bsr-contract-chatbot/src/lib/evidence/local-trace-file-writer.ts:11)
- raw traces may contain user query and answer text in the `request.userQuery` and `response.finalAnswer` fields
- `.local/traces/` is gitignored in [`.gitignore`](/home/ccod/repos/bsr-contract-chatbot/.gitignore:44)
...
- trace capture happens after the candidate answer exists and may add up to the bounded sender timeout to completion handling; the current bound is `1200ms` in [`LOCAL_TRACE_SENDER_TIMEOUT_MS`](/home/ccod/repos/bsr-contract-chatbot/src/lib/evidence/local-answer-trace.ts:13)
- send failures remain log-only because `/api/chat` catches them and logs `[chat] local-trace-capture:error` in [`src/app/api/chat/route.ts`](/home/ccod/repos/bsr-contract-chatbot/src/app/api/chat/route.ts:1844)
- invalid endpoint config is skipped with the log line `[chat] local-trace-capture:skipped invalid-endpoint-config` in [`sendLocalAnswerTrace()`](/home/ccod/repos/bsr-contract-chatbot/src/lib/evidence/local-answer-trace.ts:299)
